...
def parse_args():
    ...
    parser.add_argument(
        '--use-opencv',
        help='process actual video rather than testing mode',
        action="store_true"
    )
    parser.add_argument(
        '--rotate',
...
def main(args):
    ...
    for video_fn in manifest:
        ...
        video_id = input_bn.split("_", 1)[0] if "_" in input_src else "noid"
        ...
        if not os.path.exists(json_filename) and not args.clobber:
                ...
                if ret:
                    ...
                    for cls_id, cls_array in enumerate(cls_boxes):
                        for box in cls_array:
                            # (frame_num, cls id, prob, bbox)
                            output_json["items"].append((i, cls_id, float(box[4]), box[:4].astype(int).tolist()))
                    ...
                    # for dev to skip inference step
                    dev_data.append(cls_boxes)
                    # simple frame count
                    i += 1
